...
def hello_transformers():
    print("Hello from my_transformers.py!")
...
def scaled_dot_product_two_loop_single(
    ...
    N_x, D_q = query.shape
    N_x, D_v = value.shape
    E = torch.zeros(N_x, N_x, dtype=query.dtype, device=query.device)
    for i in range(N_x):
        for j in range(N_x):
            E[i, j] = torch.inner(query[i], key[j]) / torch.tensor(D_q).sqrt()
...
def scaled_dot_product_two_loop_batch(
    ...
    # Replace "pass" statement with your code
    N, N_x, D_q = key.shape
    E = torch.zeros(N, N_x, N_x, dtype=key.dtype, device=key.device)
    for i in range(N_x):
        for j in range(N_x):
            E[:, i, j] = torch.einsum('bi,bi->b', query[:, i], key[:, j])
    E = E / (D_q ** 0.5)
    A = torch.softmax(E, dim=-1)
    out = torch.bmm(A, value)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    E = torch.bmm(query, key.transpose(-2, -1))
    E = E / (M ** 0.5)
    if mask is not None:
        ##########################################################################
    ...
    # Replace "pass" statement with your code
    weights_softmax = torch.softmax(E, dim=-1)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        q, k, v = self.q.forward(query), self.k.forward(key), self.v.forward(value)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        self.MultiHeadAttention = nn.ModuleList([SelfAttention(dim_in, dim_out, dim_out) for _ in range(num_heads)])
        self.fc = nn.Linear(dim_out * num_heads, dim_in)
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        y = self.fc.forward(
            torch.cat([SingleHeadAttention.forward(query, key, value, mask) for SingleHeadAttention in self.MultiHeadAttention], dim=-1)
        )
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
