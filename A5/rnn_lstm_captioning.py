...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        step_dh = dh[:, t, :] + dnext_h
        step_dx, dnext_h, step_dWx, step_dWh, step_db = rnn_step_backward(step_dh, cache[t])
        dx[:, t, :] = step_dx
        dWx += step_dWx
        dWh += step_dWh
        db += step_db
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if cell_type == 'rnn' or cell_type == 'lstm':
            self.project_features = nn.Linear(input_dim * 16, hidden_dim)
        elif cell_type == 'attn':
            self.project_features = nn.Linear(input_dim, hidden_dim)

        if cell_type == 'rnn':
            self.backbone = RNN(input_dim=wordvec_dim, hidden_dim=hidden_dim)
        elif cell_type == 'lstm':
            self.backbone = LSTM(input_dim=wordvec_dim, hidden_dim=hidden_dim)
        elif cell_type == 'attn':
            self.backbone = AttentionLSTM(input_dim=wordvec_dim, hidden_dim=hidden_dim)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        image_encoder, backbone, word_embedding, fc = self.image_encoder, self.backbone, self.word_embedding, self.fc
        # transform the words in captions_in from indices to vectors
        words = word_embedding.forward(captions_in)
        # project image feature to the initial hidden state
        features = image_encoder.forward(images)
        if self.cell_type in ['rnn', 'lstm']:
            h0 = self.project_features(features.reshape(features.shape[0], -1))
            # process the sequence of input word vectors and produce hidden state vectors for all timesteps
            hidden_states = backbone.forward(words, h0)
        elif self.cell_type == 'attn':
            h0 = self.project_features(features.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
            hidden_states = backbone.forward(words, h0)
        # compute scores over the vocabulary at every timestep using the hidden states
        scores = fc.forward(hidden_states)
    ...
    def sample(self, images, max_length=15):
        ...
        for i in range(max_length):
            words_embedded = word_embedding.forward(words).reshape(N, -1)
            if self.cell_type == 'rnn':
                prev_h = backbone.step_forward(words_embedded, prev_h)
            elif self.cell_type == 'lstm':
...
class LSTM(nn.Module):
    ...
    def step_forward(
        ...
        i, f, o, g = F.sigmoid(a_i), F.sigmoid(a_f), F.sigmoid(a_o), F.tanh(a_g)
        next_c = prev_c * f + i * g
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    attn_weights = torch.zeros(N, D_a, D_a).long()
    attn = torch.zeros_like(prev_h)
    A_reshaped = A.reshape(N, H, -1)
    scores = torch.bmm(prev_h.unsqueeze(1), A_reshaped).squeeze(1) / torch.tensor(H, dtype=A.dtype).sqrt()
    attn_weights = F.softmax(scores)
    attn = torch.bmm(attn_weights.unsqueeze(1), A_reshaped.permute(0, 2, 1)).squeeze(1)
