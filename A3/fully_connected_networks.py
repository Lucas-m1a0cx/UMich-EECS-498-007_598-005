...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        #####################################################
        # Replace "pass" statement with your code
        relu = torch.zeros_like(dout)
        relu[x > 0] = 1
        dx = torch.mul(dout, relu)
        #####################################################
        #                  END OF YOUR CODE                 #
        #####################################################
        return dx
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        # num_train = X.shape[0]
        # hidden = torch.mm(X.view(num_train, -1), self.params['W1']) + self.params['b1']
        # hidden[hidden < 0] = 0
        # scores = torch.mm(hidden, self.params['W2']) + self.params['b2']
        firstLinear_and_relu, secondLinear = Linear_ReLU, Linear
        hidden, cache_hidden = firstLinear_and_relu.forward(X, self.params['W1'], self.params['b1'])
        scores, cache_scores = secondLinear.forward(hidden, self.params['W2'], self.params['b2'])
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dhidden, grads['W2'], grads['b2'] = secondLinear.backward(grad_softmax, cache_scores)
        _, grads['W1'], grads['b1'] = firstLinear_and_relu.backward(dhidden, cache_hidden)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ##################################################################
        # Replace "pass" statement with your code
        input, dropout_cache, cache_layer = X, {}, {}

        for i in range(self.num_layers - 1):
            W = self.params[f'W{i + 1}']
            b = self.params[f'b{i + 1}']
            input, cache_layer[i + 1] = Linear_ReLU.forward(input, W, b)
            if self.use_dropout:
                input, dropout_cache[i + 1] = Dropout.forward(input, self.dropout_param)
        ...
        for i in reversed(range(self.num_layers)):
            if i == self.num_layers - 1:
                ...
                if self.use_dropout:
                    upstream_grads = Dropout.backward(upstream_grads, dropout_cache[i + 1])
                upstream_grads, grads[f'W{i + 1}'], grads[f'b{i + 1}'] = Linear_ReLU.backward(upstream_grads, cache_layer[i + 1])
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, update_rule=sgd, optim_config=optim_config, lr_decay=0.95, num_epochs=50, batch_size=100, print_every=100, device='cuda')
...
def get_five_layer_network_params():
    ...
    # model achieves 100% training accuracy within 20 epochs.      #
    ################################################################
    learning_rate = 20e-2  # Experiment with this!
    weight_scale = 1e-1   # Experiment with this!
    # Replace "pass" statement with your code
...
def rmsprop(w, dw, config=None):
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    lr, decay_rate, epsilon, cache = config['learning_rate'], config['decay_rate'], config['epsilon'], config['cache']
    cache = decay_rate * cache + (1 - decay_rate) * dw * dw
    w -= lr * dw / (cache.sqrt() + epsilon)
    next_w, config['cache'] = w, cache
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    config['t'] += 1
    t = config['t']
    moment1, moment2, lr= config['m'], config['v'], config['learning_rate']
    beta1, beta2, epsilon = config['beta1'], config['beta2'], config['epsilon']
    moment1 = beta1 * moment1 + (1 - beta1) * dw
    moment1_t = moment1 / (1 - beta1 ** t)
    moment2 = beta2 * moment2 + (1 - beta2) * dw * dw
    moment2_t = moment2 / (1 - beta2 ** t)
    w -= lr * moment1_t / (moment2_t.sqrt() + epsilon)
    next_w, config['m'], config['v'] = w, moment1, moment2
