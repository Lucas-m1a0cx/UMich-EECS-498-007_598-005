...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        H_prime, W_prime = 1 + (H + 2 * pad - HH) // stride, 1 + (W + 2 * pad - WW) // stride
            ...
            for f in range(F):
                for i in range(H_prime):
                    for j in range(W_prime):
                        start_H, end_H = stride * i, stride * i + HH
                        start_W, end_W = stride * j, stride * j + WW
                        out[n, f, i, j] = (x_padded[n, :, start_H:end_H, start_W:end_W] * w[f]).sum()
                out[n, f] += b[f]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
    ...
    def backward(dout, cache):
        ...
        padding = (pad, pad, pad, pad)
        H_prime, W_prime = 1 + (H + 2 * pad - HH) // stride, 1 + (W + 2 * pad - WW) // stride
        x_padded = torch.nn.functional.pad(x, padding, "constant", 0) # Pad the input
        # Initialization
            ...
            for f in range(F):
                for i in range(H_prime):
                    for j in range(W_prime):
                        start_H, end_H = stride * i, stride * i + HH
                        start_W, end_W = stride * j, stride * j + WW
                        window = x_padded[n, :, start_H:end_H, start_W:end_W]
            ...
            for f in range(F):
                for i in range(H_prime):
                    for j in range(W_prime):
                        start_H, end_H = stride * i, stride * i + HH
                        start_W, end_W = stride * j, stride * j + WW
                        dx_padded[n, :, start_H:end_H, start_W:end_W] += dout[n, f, i, j] * w[f]             
    ...
    def forward(x, pool_param):
        ...
        for n in range(N):
            for c in range(C):
              for i in range(H_prime):
                  for j in range(W_prime):
                      start_H, end_H = stride * i, stride * i + pool_height
                      start_W, end_W = stride * j, stride * j + pool_width
                      out[n, c, i, j] = x[n, c, start_H:end_H, start_W:end_W].max()
            ...
            for c in range(C):
                for i in range(H_prime):
                    for j in range(W_prime):
                        ...
                        max = window.max()
                        mask = (window == max)
                        dx[n, c, start_H:end_H, start_W:end_W] += mask * dout[n, c, i, j]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params['W1'], self.params['b1'] = weight_scale * torch.randn(kernel_shape, dtype=dtype, device=device), torch.zeros(num_filters, dtype=dtype, device=device)
        self.params['W2'], self.params['b2'] = weight_scale * torch.randn(hidden_shape, dtype=dtype, device=device), torch.zeros(hidden_dim, dtype=dtype, device=device)
        self.params['W3'], self.params['b3'] = weight_scale * torch.randn(output_linear_shape, dtype=dtype, device=device), torch.zeros(num_classes, dtype=dtype, device=device)
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        for i in range(self.num_layers - 1):
            ...
            if weight_scale == 'kaiming':
                ...
                self.params[f'W{layer_idx}'] = weight_scale * torch.randn((output_channels, input_channels, 3, 3), dtype=dtype, device=device)

            self.params[f'b{layer_idx}'] = torch.zeros(output_channels, dtype=dtype, device=device)
            if self.batchnorm:
                self.params[f'gamma{layer_idx}'] = torch.ones(output_channels, dtype=dtype, device=device)
                self.params[f'beta{layer_idx}'] = torch.zeros(output_channels, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        upstream_grads = grad_softmax
        for i in reversed(range(self.num_layers)):
            if i == self.num_layers - 1:
                upstream_grads, grads[f'W{i + 1}'], grads[f'b{i + 1}'] = Linear.backward(upstream_grads, cache_layer[i + 1])
            else:
                if i in self.max_pools:
                    if self.batchnorm:
                        upstream_grads, grads[f'W{i + 1}'], grads[f'b{i + 1}'], grads[f'gamma{i + 1}'], grads[f'beta{i + 1}'] = Conv_BatchNorm_ReLU_Pool.backward(upstream_grads, cache_layer[i + 1])
                    else:
                        upstream_grads, grads[f'W{i + 1}'], grads[f'b{i + 1}'] = Conv_ReLU_Pool.backward(upstream_grads, cache_layer[i + 1])
                
                else:
                    if self.batchnorm:
                        upstream_grads, grads[f'W{i + 1}'], grads[f'b{i + 1}'], grads[f'gamma{i + 1}'], grads[f'beta{i + 1}'] = Conv_BatchNorm_ReLU.backward(upstream_grads, cache_layer[i + 1])
                    else:
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    #########################################################
    # Replace "pass" statement with your code
    model = DeepConvNet(num_filters=[32, 64, 128], max_pools=[0, 1, 2], dtype=dtype, device=device)
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = (x, x_hat, gamma, beta, eps, sample_mean, sample_var, mode)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
            ...
            out = gamma * x_hat + beta
            cache = (x, x_hat, gamma, beta, eps, running_mean, running_var, mode)
            ################################################################
            #                      END OF YOUR CODE                        #
            ################################################################
    ...
    def backward(dout, cache):
        ...
        mode = cache[-1]
        if mode == 'train':
            x, x_hat, gamma, beta, eps, sample_mean, sample_var = cache[:-1]
            dx_hat = dout * gamma
            dsample_var = torch.sum(-0.5 * dx_hat * (x - sample_mean) * torch.pow(sample_var + eps, -1.5), dim=0)
            dsample_mean = torch.sum(-1 * dx_hat / (sample_var + eps).sqrt(), dim=0) + dsample_var * torch.mean(-2 * (x - sample_mean), dim=0)
            ...
            x, x_hat, gamma, beta, eps, running_mean, running_var = cache[:-1]
            dbeta = torch.sum(dout, dim=0)
            dgamma = torch.sum(x_hat * dout, dim=0)
            dx = gamma /(running_var + eps).sqrt()
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
    ...
    def backward_alt(dout, cache):
        ...
        x, x_hat, gamma, beta, eps, sample_mean, sample_var = cache[:-1]
        dx_hat = dout * gamma
        dx = (dx_hat - torch.mean(dx_hat, dim=0)- x_hat * torch.mean(dx_hat * x_hat, dim=0)) / (sample_var + eps).sqrt()
        dgamma = torch.sum(dout * x_hat, dim=0)
        dbeta = torch.sum(dout, dim=0)
        #################################################################
        #                        END OF YOUR CODE                       #
