...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x = torch.arange(start // 10 * 10, stop // 10 * 10 + 1, step=10)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    last_row, third_col, first_two_rows_three_cols, even_rows_odd_cols = x[-1], x[:, 2], x[:2, :3], x[1::2, ::2]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    out = (
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    len_x = x.shape[0]
    idx = [i for i in range(len_x - 1, -1, -1)]
    y = x[idx]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x_shaped = x.view(3, 8)
    y = torch.empty_like(x_shaped)

    y[0, :] = torch.cat((x[0:4], x[12:16]))
    y[1, :] = torch.cat((x[4:8], x[16:20]))
    y[2, :] = torch.cat((x[8:12], x[20:24]))
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    min_values = torch.min(y, dim=1)[0]
    min_masks = (y == min_values)
    y[min_masks] = 0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for i in range(loop_times):
        tensor_list.append(torch.mm(x[i], y[i]))
    z = torch.stack(tensor_list, dim=0)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    col_sums, num_rows = torch.sum(x, dim=0), x.size(0)
    col_means = col_sums / num_rows

    # Calculate the standard deviation of each column
    deviation = x - col_means
    squared_deviation = deviation ** 2
    col_variances = squared_deviation.sum(dim=0) / num_rows
    standard_deviation = col_variances.sqrt()
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    end_indices = torch.cumsum(ls, dim=0)
    start_indices = end_indices - ls

    mask = torch.arange(concatenated.size(0)).unsqueeze(0).repeat(len(ls), 1)
    mask = (mask >= start_indices.unsqueeze(1)) & (mask < end_indices.unsqueeze(1))

    sums = torch.sum(mask.float() * concatenated, dim=1)
